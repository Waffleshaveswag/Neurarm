...
const CalibModule = {
  ...
  init() {
    this.chart = mkChart('chart-calib-emg',
      [{ color: '#0a8a8a', fill: 'rgba(10,138,138,0.08)', width: 1.5 }],
      -250, 250
    );
  },
...
const TrackerModule = {
  ...
  init() {
    this.charts.emg = mkChart('chart-tracker-emg', [
      { color: '#0a8a8a', fill: 'rgba(10,138,138,0.06)', width: 1.5 },
      { color: '#076060', fill: false, width: 1.2 }
    ], -250, 250);

    this.charts.imu = mkChart('chart-tracker-imu', [
      { color: '#0a8a8a', fill: 'rgba(10,138,138,0.08)', width: 1.5 },
      { color: '#2dd6d6', fill: false, width: 1, borderDash: [4,4] }
    ], 0, 120);

    this.charts.fes = mkChart('chart-tracker-fes', [
      { color: '#076060', fill: 'rgba(7,96,96,0.12)', width: 1.5 }
    ], 0, 55);
  },
...
const DashModule = {
  ...
  init() {
    this.charts.tremor = new Chart(document.getElementById('chart-tremor-trend'), {
      type: 'line',
      data: { labels: [], datasets: [{ data: [], borderColor: '#16a34a', backgroundColor: 'rgba(22,163,74,0.06)', borderWidth: 2, tension: 0.4, fill: true, pointRadius: 3, pointBackgroundColor: '#16a34a' }] },
    ...
    this.charts.bar = new Chart(document.getElementById('chart-session-bar'), {
      type: 'bar',
      data: { labels: [], datasets: [{ data: [], backgroundColor: 'rgba(22,163,74,0.55)', borderColor: '#16a34a', borderWidth: 1, borderRadius: 2 }] },
      options: {
        ...CHART_DEFAULTS,
  ...
  refresh() {
    ...
    const name = patient ? `, ${patient.name.split(' ')[0]}` : '';
    ...
    if (sessions.length > 0) {
      ...
      document.getElementById('stat-tremor').textContent = latest.tremorIndex;
      document.getElementById('stat-velocity').textContent = latest.velocityScore + '%';
    }

    if (calib) {
      document.getElementById('stat-calib').textContent = 'Complete';
      document.getElementById('stat-calib-date').textContent = `Last: ${formatDateShort(calib.completedAt)}`;
    }

    // Sidebar patient
    const patientTag = document.getElementById('sidebar-patient');
    patientTag.textContent = patient ? patient.name : 'No patient';

    // Recent sessions list
    const listEl = document.getElementById('recent-sessions-list');
    if (sessions.length === 0) {
      listEl.innerHTML = '<div class="empty-state">No sessions yet. Complete calibration to start.</div>';
    } else {
...
const HistoryModule = {
  ...
  exportCSV() {
    ...
    a.href = url; a.download = `neurobridge-sessions-${new Date().toISOString().split('T')[0]}.csv`;
...
const ProfileModule = {
  ...
  save() {
    ...
    Store.savePatient(p);
    document.getElementById('sidebar-patient').textContent = p.name || 'No patient';
    DashModule.refresh();
    // Flash feedback
...
const AIModule = {
  ...
  analyze() {
    ...
    const condition = patient ? patient.condition : 'parkinsons';
  ...
  _buildRecs(condition, avgMA, peakMA, tremorHz, sessions) {
    ...
    if (condition === 'parkinsons' || condition === 'als') {
...
function initIntro() {
  ...
  // Background animated squares grid
  const ctx  = canvas.getContext('2d');
  const SQ   = 28, GAP = 4, STEP = SQ + GAP;
  let cols, rows, tiles, animId;
  ...
  function buildTiles() {
    ...
    for (let r = 0; r < rows; r++) {
      for (let c = 0; c < cols; c++) {
        tiles.push({
          x: c * STEP, y: r * STEP,
          phase: (c * 0.6 + r * 0.9) + Math.random() * 3,
  ...
  function drawTiles() {
    ...
    for (const tile of tiles) {
      const wave = 0.5 + 0.5 * Math.sin(t * tile.speed + tile.phase);
      const a = 0.03 + wave * 0.13;
      const g = Math.round(138 + wave * 50);
      ctx.fillStyle = `rgba(10,${g},138,${a})`;
      ctx.fillRect(tile.x, tile.y, SQ, SQ);
    }
    animId = requestAnimationFrame(drawTiles);
  }
...
// ── Intel squares animation ───────────────────────────────────────────────────
function initIntelSquares() {
  const canvas = document.getElementById('intel-squares-canvas');
  if (!canvas) return;
  const ctx = canvas.getContext('2d');

  const SQ = 9;   // square size px
  const GAP = 3;  // gap px
  const STEP = SQ + GAP;
    ...
    for (let r = 0; r < rows; r++) {
      for (let c = 0; c < cols; c++) {
        squares.push({
          x: c * STEP,
          y: r * STEP,
          // brightness: 0 = off, 1 = full Intel blue
          bright: Math.random() * 0.25,
          // each square has its own phase offset for the wave
          phase: (c + r) * 0.38 + Math.random() * 2,
  ...
  function draw() {
    ...
    for (const sq of squares) {
      // Wave: combination of a diagonal wave + random flicker
      const wave = 0.5 + 0.5 * Math.sin(t * sq.speed + sq.phase);
      const bright = Math.max(0, Math.min(1, wave * 0.9 + sq.bright * 0.1));

      // Green at full brightness, dark forest at zero
      const r = Math.round(5   + bright * 17);   // 5→22
      const g = Math.round(46  + bright * 117);  // 46→163
      const b = Math.round(16  + bright * 58);   // 16→74
      const a = 0.10 + bright * 0.70;
